...
LOG=/vol/bitbucket/sb1522/vm-backup/NIR-RGB/experiments/ablation_08_foundation_ensemble_aggressive.train.log
mkdir -p "$(dirname "$LOG")"
...
./.venv-kaist/bin/python -u scripts/train.py \
  --config configs/ablation_08_foundation_ensemble_aggressive.yaml \
  2>&1 | tee -a "$LOG"
